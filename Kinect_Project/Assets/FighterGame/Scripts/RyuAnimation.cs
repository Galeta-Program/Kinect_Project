...
{
    public RyuAnimationEvents playerAEs;
    ...
    {      
        ...
        {
            if (keyCodeIsTrigger[KeyCodeSF.SquatDown] && keyCodeIsTrigger[KeyCodeSF.Jump] && keyCodeIsTrigger[KeyCodeSF.HighKick] && groundCheck && playingTrigger == "SquatDown")
            {
            ...
            else if((keyCodeIsTrigger[KeyCodeSF.Forward] || keyCodeIsTrigger[KeyCodeSF.Backward]) && keyCodeIsTrigger[KeyCodeSF.SquatDown] && keyCodeIsTrigger[KeyCodeSF.HighPunch] && groundCheck && playingTrigger == "SquatDown")
            {
                if (transform.parent.tag == "Player1" && gameUIControl.player1_QigongNum >= 1)
                {
                    playingTrigger = "Ultimate1";
                    playingTriggerTimer.Start(0.7f);
                    ultimateTimer.Start(0.7f);
                    isIdle = false;
                    speedBonus = 1;
                    gameUIControl.player1_QigongNum--;
                    gameUIControl.player1_QigongPercent = 0;
                    ultimateJumpTimer.Start(0.2f);
                }
            ...
            else if (keyCodeIsTrigger[KeyCodeSF.LightPunch] && keyCodeIsTrigger[KeyCodeSF.HighPunch] && chackTime)
            {
                if (groundCheck && !isSquatDwon)
                {                
                    if (Vector3.Distance(gameManager.GetOpponent(transform.parent.tag).transform.position, transform.position) > 0.5f)
                    {
            ...
            else if (keyCodeIsTrigger[KeyCodeSF.LightPunch] && !keyCodeIsTrigger[KeyCodeSF.HighPunch])
            {
                if (groundCheck && !isSquatDwon)
                {
                    ...
                    {
                        playingTrigger = "CloseLightPunch";
                        playingTriggerTimer.Start(0.4f);
                    }
                }
                else if (!groundCheck && !isSquatDwon)
        ...
        if (turnAroundTimer.isTimeOut() && groundCheck)
            transform.localScale = new Vector3((gameManager.GetOpponent(transform.parent.tag).transform.position.x - 0.1 > transform.position.x - playerAEs.additionalPos.x ? 1 : -1), 1, 1);
        ...
        {
            canBeHurt = false;
            if (playingTrigger == "Ultimate1" && ultimateJumpTimer.isTimeOut())
            {
                gravity = -6.5f;
                ultimateJumpTimer.Start(10f);
            }
    ...
    void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (collidedObject != null)
        {
            ...
            if (!groundCheck && canBeHurt)
            {
                nowKnockedDownCount = 5;
            }

            if (nowKnockedDownCount != 0 && !isDefensing && canBeHurt)
            {
            ...
            if (!knockedDownTimer.isTimeOut() && !isDefensing && canBeHurt)
            {
                ...
                playingTriggerTimer.Start(1.6f);
                gravity = -6.5f;
            }
            else if (isSquatDwon && !isDefensing && canBeHurt)
            {
                ...
                playingTriggerTimer.Start(0.3f);
            }
            else if (!isSquatDwon && !isDefensing && canBeHurt)
            {
